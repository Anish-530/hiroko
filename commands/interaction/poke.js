        ...
        let scbaq = new Discord.MessageEmbed()
                ...
                if (message.mentions.members.first() !== undefined) {
                    ...
                    if (message.mentions.members.first().id === bot.user.id) {
                     ...
                     return message.channel.send(`*Why poke me? >~<*`)
                  
                    }
                }
                const pokee = await message.channel.send("\`Poking...\`")
        if(message.author === mentionedddMembere222baq) {
            Tenor.Search.Random("anime poke", "1").then(Resultsrbsq => {
            ...
            Tenor.Search.Random("anime poke", "1").then(Resultsrbaq => {
