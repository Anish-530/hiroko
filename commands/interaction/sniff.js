        ...
        if(message.author === mentionedddMembere222bbb) {
            Tenor.Search.Random("anime sniff", "1").then(Resultsrbb => {
                Resultsrbb.forEach(Postrbb => {
                    ...
                    scbb.setDescription(`**${message.guild.members.cache.get(message.author.id).displayName}** sniff themselves 👃`)
            ...
            Tenor.Search.Random("anime sniff", "1").then(Resultsrbb => {
                Resultsrbb.forEach(Postrbb => {
                    ...
                    scbb.setColor(0x2f3136)
                    scbb.setDescription(`**${message.guild.members.cache.get(message.author.id).displayName}** sniffs **${message.guild.members.cache.get(mentionedddMembere222bbb.id).displayName}** 👃`)
                    scbb.setImage(gifrbb.url)
                    scbb.setTimestamp(new Date())
                    scbb.setFooter("Hiroko", bot.user.avatarURL())
                    return shoott.edit("\t", scbb)
