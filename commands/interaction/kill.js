...
module.exports={
    ...
    run: async(bot, message, args)=>{
        ...
        if (mentionedddMember111.id === message.guild.me.id) return message.channel.send(`*Dies x_x*`)
        if (mentionedddMember111.id === message.author.id) {
            Tenor.Search.Random("anime wasted", "1").then(Results111 => {
                Results111.forEach(Post111 => {
                    ...
                    ki11.setDescription(`**${message.author.username}** killed themselves 🤕`)
